...
namespace MyRecipeBook.Application.UseCases.Recipe
{
    public class RecipeValidator:AbstractValidator<RequestRecipeJson>
    {
        public RecipeValidator()
        {
            ...
            {
                instructionRule.RuleFor(instruction => instruction.Step).GreaterThanOrEqualTo(1).WithMessage(ResourceMessagesExceptions.NON_NEGATIVE_INSTRUCTION_STEP);
            ...
            RuleFor(recipe => recipe.Instructions).Must(instructions => instructions.Select(i => i.Step).Distinct().Count() == instructions.Count).WithMessage(ResourceMessagesExceptions.TWO_OR_MORE_INSTRUCTIONS_SAME_ORDER);
        }
    }
}
